/* 3S Verse POS System - Electron main process
   - Loads the local UI
   - Auto Google-Drive folder local backup (bonus, silent)
   - WhatsApp auto-send: copies invoice image to clipboard, opens WhatsApp Desktop
     on the customer's chat, and auto-pastes + Enter via PowerShell SendKeys. */
...
/* WhatsApp auto-send (Windows + WhatsApp Desktop). Copies image, opens chat, pastes, sends. */
ipcMain.handle('wa:send', async (e, { phone, caption, pngDataUrl }) => {
  try {
    const tmp = os.tmpdir();
    const imgPath = path.join(tmp, 'verse_wa_' + Date.now() + '.png');
    const capPath = path.join(tmp, 'verse_wa_' + Date.now() + '.txt');
    const b64 = (pngDataUrl || '').split(',')[1] || '';
    fs.writeFileSync(imgPath, Buffer.from(b64, 'base64'));
    fs.writeFileSync(capPath, caption || '', 'utf8');
    if (process.platform !== 'win32') {
      // Non-Windows fallback: copy image to clipboard and open chat
      try { clipboard.writeImage(nativeImage.createFromPath(imgPath)); } catch (x) {}
    ...
    const ps = [
      ...
      'if($cap){ [System.Windows.Forms.Clipboard]::SetText($cap); Start-Sleep -Milliseconds 500; $ws.SendKeys(\"^v\"); Start-Sleep -Milliseconds 600 }',
      "$ws.SendKeys('{ENTER}')",
      'Start-Sleep -Milliseconds 800'
    ].join('\n');
    const scriptPath = path.join(tmp, 'verse_wa_' + Date.now() + '.ps1');
